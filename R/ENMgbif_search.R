...
#' @description
#' 使用rgbif包从GBIF下载最多500条物种发生数据并进行常规数据清洗. 主要用于物种分布建模的数据演示使用. 如果想要获得完整下载, 请使用\code{\link[TBlabENM]{ENMgbif_download}}.
#' @details
...
#' @seealso [ENMgbif_download()]
#' @param spnames character string or character vector. 要下载物种的学名.
#' @param range character string. 用国家代码(ISO 3166-1 alpha2)或WKT多边形(逆时针)表示的地理范围. 如果为NULL，则在全球范围下载.
...
#' @importFrom rgbif occ_search pred_default  pred_in  pred_or  pred pred_lt pred_not
#' @importFrom rgbif pred_isnull occ_download_wait occ_download_get occ_download_import
#' @importFrom purrr map_int
#' @importFrom dplyr filter distinct select rename
...
#'
#' @examples
#' \donttest{ENMgbif_search(spnames = c("Phoebe formosana", "Decaisnea insignis", "Ginkgo biloba"),
...
ENMgbif_search <- function(spnames, range = NULL, uncertainty = 10000, rWCVP = TRUE, dist = 1,
  ...
  if (rWCVP == TRUE) {
    write.csv(occs_i[-11], paste0(outdir, "/", filename, ".csv"), row.names = FALSE)
    return(occs_i[-11])
  } else {
    write.csv(datasel[-11], paste0(outdir, "/", filename, ".csv"), row.names = FALSE)
    return(datasel[-11])
  }

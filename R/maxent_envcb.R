...
#'              device = "jpeg",
#'              outdir = "F:/4")
maxent_envcb <- function(parameters, x = NULL, resultdir, wordsize = 1, device = "jpeg", outdir = NULL) {
  #检查输入变量
  if(is.null(x)){x <- 1:nrow(parameters)}
  if (file.exists(resultdir)==FALSE){
    stop("Resultdir not find.")}
  #检查x值是否正确
  if(max(x)>nrow(parameters)){stop("x exceeds the total number of species")}
  ...
  names(maxselectev) <- c("species", "occurrence", "Variable contribution size",
    ...
    ss <- spdata %>%
      mutate(nenv = map_dbl(.x = env, .f = function(x){
        str_count(x, ",")+1
      })) %>%
      mutate(path = map_chr(.x = species, .f = function(x){
        list.files(paste0(resultdir, "/", x), full.names = TRUE,
                          pattern = "maxentResults.csv$" , recursive = TRUE )[1]
      })) %>%
      mutate(par = map(.x = path, .f = function(x){read.csv(x)})) %>%
      #提取变量重要性
      mutate(ev_cb = map2(.x = par, .y = nenv, .f = function(x, y){
        dt <- as.data.frame(t(x[nrow(x), 12:(11+y), drop = FALSE]))
        names(dt) <- "value"
        dplyr::arrange(dt, dplyr::desc(value))
      })) %>%
      mutate(var = map(.x = ev_cb, .f = function(x){
        site <- unlist(stringr::str_split(rownames(x), "\\."))
        site <- site[seq(1,length(site),2)]
      })) %>%
      mutate(v1 = map2_chr(.x = var, .y = ev_cb, .f = function(x,y){
        paste(paste0(x, "(",y$value, "%)", collapse=","))
      })) %>%
      mutate(v2 = map_chr(.x = var, .f = function(x){
        paste(paste0(x, collapse=","))
      })) %>%
      mutate(v3 = map2_chr(.x = var, .y = ev_cb, .f = function(x,y){
        paste(paste0(x, " ", y$value, collapse=","))
      }))
    ...
    #保存本地
    dir.create(paste0(outdir, "/TBlabENM/Variable_contribution"), showWarnings = FALSE)
    utils::write.csv(maxselectev[1:3], row.names = FALSE, paste0(outdir,"/TBlabENM/Variable_contribution/Variable.csv"))
...
#统计每个变量出现次数，按照次数指定因子水平
  ...
  ggplot2::ggsave(plot = p, filename = paste0("Variable contribution statistics.", device),
         path = paste0(outdir,"/TBlabENM/Variable_contribution"),
         scale=1, dpi = "print", width = 10,
         height = 7)
  ...
  write.csv(ee1, paste0(outdir, "/TBlabENM/Variable_contribution/Variable Percent contribution.csv"), row.names = FALSE)
  ...
  ggplot2::ggsave(plot = p, filename = paste0("Variable Percent contribution.", device),
                  path = paste0(outdir,"/TBlabENM/Variable_contribution"),
                  scale=1, dpi = "print", width = 10,
                  height = 7)
  }

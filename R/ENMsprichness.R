...
ENMsprichness <- function(d, x = NULL, booleandir, key = NULL,
  ...
  if (length(x) == 1) {
    stop("It has to be more than 2 species, otherwise it doesn't make sense.")
  }
  if (file.exists(booleandir) == FALSE) {
    stop(paste0("Dir '", booleandir, "' not exists."))
  }
  if (length(bioindex) != 1) {
    stop("length bioindex must be 1.")
  }
  ...
  if (length(rasterlist) == 0) {
    stop("No .tif files were found under booleandir.")
  }
  # 如果指定key，则提取想要的时期
